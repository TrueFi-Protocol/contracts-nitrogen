...
build_docker() {
    mkdir -p build
    docker build -f spec/docker/Dockerfile ../.. -t verify_nitrogen
}
...
spawn_single_container() {
    conf="$1"
    id=$(docker run -d -e CERTORAKEY -e BRANCH -e ALL_SPEC -v $(pwd)/build:/root/build:ro verify_nitrogen $conf $SANITY $TYPECHECK_ONLY)
    echo "Spawned $id"
    ids="$ids $id"
}

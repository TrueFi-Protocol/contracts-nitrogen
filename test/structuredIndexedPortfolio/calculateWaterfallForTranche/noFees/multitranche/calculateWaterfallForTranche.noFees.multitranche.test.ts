...
describe('StructuredIndexedPortfolio.calculateWaterfallForTranche.noFees.multitranche', () => {
  ...
  describe('with assets deposited', () => {
    ...
    describe('live', () => {
      it('senior underflow, junior empty, equity empty', async () => {
        ...
        expect(await portfolio.calculateWaterfallForTranche(0)).to.eq(Zero)
        expect(await portfolio.calculateWaterfallForTranche(1)).to.eq(Zero)
        expect(await portfolio.calculateWaterfallForTranche(2)).to.eq(newVirtualTokenBalance)
      })
      ...
      it('senior full, junior empty, equity empty', async () => {
        ...
        expect(await portfolio.calculateWaterfallForTranche(0)).to.eq(Zero)
        expect(await portfolio.calculateWaterfallForTranche(1)).to.eq(Zero)
        expect(await portfolio.calculateWaterfallForTranche(2)).to.eq(expectedSeniorTrancheValue)
      })
      ...
      it('senior full, junior underflow, equity empty', async () => {
        ...
        expect(await portfolio.calculateWaterfallForTranche(0)).to.eq(Zero)
        expect(await portfolio.calculateWaterfallForTranche(1)).to.eq(expectedJuniorTrancheValue)
        expect(await portfolio.calculateWaterfallForTranche(2)).to.eq(expectedSeniorTrancheValue)
      })
      ...
      it('senior full, junior full, equity surplus', async () => {
        ...
        expect(await portfolio.calculateWaterfallForTranche(0)).to.eq(expectedEquityTrancheValue)
        expect(await portfolio.calculateWaterfallForTranche(1)).to.eq(expectedJuniorTrancheValue)
        expect(await portfolio.calculateWaterfallForTranche(2)).to.eq(expectedSeniorTrancheValue)
      })
    ...
    it('closed', async () => {
      ...
      expect(await portfolio.calculateWaterfallForTranche(0)).to.eq(tranchesVirtualTokenBalances[0])
      expect(await portfolio.calculateWaterfallForTranche(1)).to.eq(tranchesVirtualTokenBalances[1])
      expect(await portfolio.calculateWaterfallForTranche(2)).to.eq(tranchesVirtualTokenBalances[2])
    })
  })
})

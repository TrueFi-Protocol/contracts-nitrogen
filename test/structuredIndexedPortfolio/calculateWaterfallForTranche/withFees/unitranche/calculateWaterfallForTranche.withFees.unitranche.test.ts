...
import { calculateInterest, calculateTranchesValuesAfterTime, createAndRegisterInvestment, ONE_DAY_IN_SECONDS, startAndGetTimestamp, sumArray } from 'utils'
...
describe('StructuredIndexedPortfolio.calculateWaterfallForTranche.withFees.unitranche', () => {
  ...
  describe('with assets deposited', () => {
    ...
    it('live', async () => {
      ...
      expect(await unitranchePortfolio.calculateWaterfallForTranche(0)).to.eq(expectedEquityAssets)
    })
    ...
    it('live, investment gained value', async () => {
      ...
      const [expectedEquityAssets] = await calculateTranchesValuesAfterTime(unitranchePortfolio, tranchesValuesAfterDeposit, increaseValueTimeElapsed, portfolioValueAfterIncrease)

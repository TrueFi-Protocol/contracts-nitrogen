...
describe('StructuredIndexedPortfolio.calculateWaterfallForTrancheWithoutFee.unitranche', () => {
  const loadFixture = setupFixtureLoader()

  it('index out of bounds', async () => {
    const { unitranchePortfolio } = await loadFixture(structuredIndexedPortfolioFixture)
    await expect(unitranchePortfolio.calculateWaterfallForTrancheWithoutFee(1)).to.be.revertedWith('SIP: Tranche index out of bounds')
  })

  describe('no assets deposited', () => {
    it('capital formation', async () => {
      const { unitranchePortfolio } = await loadFixture(structuredIndexedPortfolioFixture)
      const waterfall = await unitranchePortfolio.calculateWaterfallForTrancheWithoutFee(0)
      expect(waterfall).to.eq(Zero)
    })
  ...
  describe('with assets deposited', () => {
    it('capital formation', async () => {
      ...
      const waterfall = await unitranchePortfolio.calculateWaterfallForTrancheWithoutFee(0)
      expect(waterfall).to.eq(equityDepositAssets)
    })
    ...
    it('live, investment gained value', async () => {
      ...
      expect(await unitranchePortfolio.calculateWaterfallForTrancheWithoutFee(0)).to.eq(expectedUnitrancheValue)
    })
    ...
    it('closed', async () => {
      ...
      const waterfall = await unitranchePortfolio.calculateWaterfallForTrancheWithoutFee(0)
      expect(waterfall).to.eq(equityTrancheVirtualTokenBalance)
    })
  })
})
